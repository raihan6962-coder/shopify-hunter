...
# ─────────────────────────────────────────────────────────────────────────────
# PHASE 1: NICHE & NEW STORE SCRAPING (1 WEEK FILTER)
# ─────────────────────────────────────────────────────────────────────────────
def scrape_new_niche_stores(keyword, serpapi_key):
    """
    শুরুতেই Niche (কিওয়ার্ড) এবং 'Past 1 Week' ফিল্টার দিয়ে স্টোর কালেক্ট করবে।
    """
    urls = set()
    kw_clean = keyword.lower().replace(' ', '')
    log(f"   [Scraper] Fetching ONLY NEW (Past 7 Days) stores for Niche: '{keyword}'...", "INFO")

    # 1. URLScan (Strictly last 7 days + Keyword)
    try:
        urlscan_url = f"https://urlscan.io/api/v1/search/?q=domain:myshopify.com AND date:>now-7d AND {kw_clean}&size=500"
    ...
    # 2. crt.sh (SSL Certs with keyword in domain - inherently new setups)
    try:
    ...
    # 3. SerpAPI (Google - Strictly Past Week 'qdr:w')
    if serpapi_key:
        queries = [
        ...
        for q in queries:
            ...
            for start in [0, 100]: # First 2 pages
                try:
    ...
    log(f"   ✅ Found {len(urls_list)} NEW Niche-specific stores to test!", "SUCCESS")
    return urls_list
...
def check_store_target(base_url, session, keyword):
    ...
    try:
        ...
        # 🚨 NICHE CHECK: ডোমেইন বা হোমপেজে কিওয়ার্ড আছে কিনা চেক করবে
...
def _run():
    ...
    log("━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━", "INFO")
    log("🚀 PHASE 1 — FETCHING NEW NICHE STORES (1 WEEK FILTER)", "SUCCESS")
    log("━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━", "INFO")
    ...
    for kw_row in ready_kws:
        ...
        # 1. শুরুতেই Niche এবং 1 Week ফিল্টার দিয়ে স্টোর কালেক্ট করবে
        try:
            store_urls = scrape_new_niche_stores(keyword, serpapi_key)
        except Exception as e:
        ...
        for idx, url in enumerate(store_urls):
            ...
            try:
                ...
                if not result.get("is_lead"):
                    ...
                    # যদি কিওয়ার্ড না থাকে, তাহলে টার্মিনালে স্প্যাম করবে না
